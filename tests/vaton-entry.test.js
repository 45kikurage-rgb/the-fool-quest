...
test('名前からARUNO自動同期を開き、未導入時はGPT利用状況へ移動できる', () => {
  const html = fs.readFileSync(path.join(__dirname, '../index.html'), 'utf8');
  assert.match(html, /id="open-csv"[\s\S]*href="intent:\/\/sync\/start#Intent;/);
  assert.match(html, /scheme=arunoassist;package=com\.aruno\.assist/);
  assert.match(html, /browser_fallback_url=https%3A%2F%2Fchatgpt\.com%2Fcodex%2Fsettings%2Fusage/);
